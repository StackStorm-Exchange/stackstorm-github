...
class CreateReleaseAction(BaseGithubAction):
    def run(
        ...
        release = self._request(
            "GET", "/repos/{}/releases/latest".format(repository), None, self.token, enterprise
        )
        ...
        tag_name = "v{}.{}.{}".format(major, minor, patch)
        ...
        release = self._request(
            "POST", "/repos/{}/releases".format(repository), payload, self.token, enterprise
        )

        ts_published_at = time.mktime(
            datetime.datetime.strptime(release["published_at"], "%Y-%m-%dT%H:%M:%SZ").timetuple()
        )
