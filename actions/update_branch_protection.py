...
__all__ = ["UpdateBranchProtectionAction"]
...
class UpdateBranchProtectionAction(BaseGithubAction):
    def run(
        ...
        if not required_pull_request_reviews:
            ...
            dismiss_stale_reviews = required_pull_request_reviews[
                "dismiss_stale_reviews"
            ]
            require_code_owner_reviews = required_pull_request_reviews[
...
if __name__ == "__main__":
    ...
    act = UpdateBranchProtectionAction(
        config={"token": GITHUB_TOKEN, "github_type": "online"}
    )
    res = act.run(
        user=GITHUB_ORG,
        repo=GITHUB_REPO,
